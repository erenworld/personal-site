	...
	if (filename) {
		...
		parsed.walk(parsed.result.ast, (node, _, ctx) => {
			...
			if (isJSXElement(node)) {
				if (node.openingElement.name.type === "JSXIdentifier" && node.openingElement.name.name === "script" && node.children.length && !node.openingElement.attributes.some((attr) => attr.type === "JSXAttribute" && attr.name.type === "JSXIdentifier" && attr.name.name === "type" && attr.value?.type === "Literal" && /json$|importmap/i.test(String(attr.value.value)))) shared.addClientScript(code, node, parsed);
			}
		});
